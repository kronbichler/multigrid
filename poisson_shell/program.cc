...
namespace multigrid
{
  ...
  {
  public:
    Coefficient ()  : Function<dim,Number>() {}
  ...
  Number Coefficient<dim,Number>::value (const Point<dim,Number>  &p,
                                         const unsigned int) const
  {
    Number prod = 1.0;
    for(int e = 0; e < dim; ++e) {
  ...
  template <int dim, typename Number>
  Tensor<1,dim,Number> Coefficient<dim,Number>::gradient (const Point<dim,Number>  &p,
                                                          const unsigned int) const
  {
                                                         ...
                                                         const unsigned int n_post_smooth)
  {
    ...
    MultigridSolver<dim, degree_finite_element, vcycle_number, full_number>
      solver(dof_handler, analytic_solution, RightHandSide<dim>(),
             Coefficient<dim>(), n_pre_smooth, n_post_smooth, n_mg_cycles);
    ...
    if (Utilities::MPI::this_mpi_process(MPI_COMM_WORLD)==0)
      {
        ...
        convergence_table.evaluate_convergence_rates("fmg_L2error", "cells",
