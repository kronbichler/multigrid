...
namespace multigrid
{
  ...
  template <int dim, int fe_degree, typename number>
  void
  MinimalSurfaceOperator<dim,fe_degree,number>::
  evaluate_coefficient (const bool first_time,
                        LinearAlgebra::distributed::Vector<number> &solution)
  {
    this->merged_coefficient.resize(this->data->n_cell_batches() *
        ...
        for (unsigned int q=0; q<fe_eval.n_q_points; ++q)
          {
            Tensor<2,dim,VectorizedArray<number>> tensor =
              first_time ? unit_symmetric_tensor<dim,VectorizedArray<number> >() :
    ...
    for (unsigned int cell=0; cell<this->data->n_cell_batches(); ++cell)
      {
        ...
        for (unsigned int q=0; q<phi.n_q_points; ++q)
          phi.submit_gradient (first_time?-phi_nodirichlet.get_gradient(q):
                               -phi_nodirichlet.get_gradient(q) / std::sqrt(1.+phi_nodirichlet.get_gradient(q).norm_square()),
                               q);
  ...
  void LaplaceProblem<dim>::setup_system ()
  {
    ...
    time_details << "Distribute DoFs & B.C.                "
    ...
    setup_time += time.wall_time();
    time_details << "MG build transfer time                "
                 << time.wall_time() << " s\n";
  ...
  void LaplaceProblem<dim>::solve (const bool first_time)
  {
    ...
    typedef PreconditionChebyshev<LevelMatrixType,LinearAlgebra::distributed::Vector<level_number> > SmootherType;
    ...
    for (unsigned int level = 0; level<triangulation.n_global_levels(); ++level)
      {
        ...
        //pcout << coefficient_solutions[level].l2_norm() << " " << mg_matrices[level].get_matrix_diagonal_inverse()->get_vector().l2_norm() << std::endl;
    ...
    mg::Matrix<LinearAlgebra::distributed::Vector<level_number> > mg_interface(mg_interface_matrices);

    Multigrid<LinearAlgebra::distributed::Vector<level_number> > mg(mg_matrix,
                                                             mg_coarse,
                                                             mg_transfer,
                                                             mg_smoother,
                                                             mg_smoother);
    ...
    PreconditionMG<dim, LinearAlgebra::distributed::Vector<level_number>,
    ...
    //std::vector<LinearAlgebra::distributed::Vector<level_number> > vectors(triangulation.n_global_levels());
    //vectors.back() = system_rhs;
    //pcout << vectors.back().l2_norm() << " ";
    //for (unsigned int level=triangulation.n_global_levels()-1; level > 0; --level)
    // {
    ...
    search_direction = 0;
    cg.solve (system_matrix, search_direction, system_rhs, /*PreconditionIdentity()*/
              preconditioner);
    ...
    time_residual += time.wall_time();
    pcout << "Residual norm: " << initial_residual_norm << " in " << n_steps << " steps to " << final_residual_norm << std::endl;
    ...
    for (unsigned int cycle=0; cycle<9-dim; ++cycle)
      {
        ...
        if (cycle == 0)
        ...
        else
          {
            ...
            // parallel::distributed::SolutionTransfer<dim,LinearAlgebra::distributed::Vector<number> > solution_transfer(dof_handler);
