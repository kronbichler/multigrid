 ...
 * coefficient with the full multigrid cycle and a conjugate gradient method
 * preconditioned by a V-cycle.
 */
...
namespace Step37
{
  ...
  public:
    ...
    virtual Tensor<1,dim,VectorizedArray<double>> gradient (const Point<dim,VectorizedArray<double>>  &p,
							    const unsigned int  component = 0) const;

    virtual double laplacian (const Point<dim>   &p,
			      const unsigned int  component = 0) const;

    virtual VectorizedArray<double> laplacian (const Point<dim,VectorizedArray<double>>   &p,
					       const unsigned int  component = 0) const;
  };
  ...
  template <int dim>
  Tensor<1,dim,VectorizedArray<double>> Solution<dim>::gradient (const Point<dim,VectorizedArray<double>>  &p,
								 const unsigned int  ) const
  {
  ...
  template <int dim>
  VectorizedArray<double> Solution<dim>::laplacian (const Point<dim,VectorizedArray<double>>  &p,
						    const unsigned int) const
  {
    return -2*2*numbers::PI*2*numbers::PI*std::sin(2*numbers::PI*(p[0] + p[1]));
  }
  ...
  private:
    ...
    Coefficient<dim,double> coefficient;
  public:
    ...
    virtual VectorizedArray<double> value (const Point<dim,VectorizedArray<double>>   &p,
					   const unsigned int  component = 0) const;
  };
  ...
  template <int dim>
  VectorizedArray<double> RightHandSide<dim>::value (const Point<dim,VectorizedArray<double>>   &p,
						     const unsigned int  ) const
  {
    return -(solution.laplacian(p)*coefficient.value(p)
	     + coefficient.gradient(p)*solution.gradient(p));
  ...
  void LaplaceProblem<dim,degree_finite_element>::setup_system ()
  {
    ...
    pcout << "Number of degrees of freedom: "
	  << dof_handler.n_dofs() << " with degree " << fe.degree << std::endl;
  ...
  public:
      ...
      n_cycles (n_cycles)
    {
	  ...
	  /*
	  FullMatrix<double> mat(matrix[level].n()-level_constraints.n_constraints(),
				 matrix[level].n()-level_constraints.n_constraints());
	  for (unsigned int i=0, ii=0; i<matrix[level].n(); ++i)
	    if (!level_constraints.is_constrained(i))
	      {
		Vector<Number> v(matrix[level].n());
		v(i) = 1.;
		defect[level] = v;
		matrix[level].vmult(defect2[level], defect[level]);
		defect2[level].copyToHost(v);
		for (unsigned int j=0, jj=0; j<matrix[level].n(); ++j)
		  if (!level_constraints.is_constrained(j))
		    mat(ii,jj++) = v(j);
		ii++;
	      }
	  mat.print_formatted(std::cout, 5, 12, true, "0");
	  */
	  ...
	  const std::function< void(const MatrixFree< dim, Number2> &, Vector<Number2> &, const Vector<Number2> &, const std::pair< unsigned int, unsigned int > &)> my_func
	    ...
	    {
	      Coefficient<dim,Number2> coefficient_function;
	      ...
	      for (unsigned int cell=cell_range.first; cell<cell_range.second; ++cell)
		{
		  ...
		  for (unsigned int q=0; q<phi.n_q_points; ++q)
		      ...
		      for (unsigned int v=0; v<VectorizedArray<Number2>::n_array_elements; ++v)
			{
		      ...
		      phi.submit_gradient(-coefficient_function.value(phi.quadrature_point(q)) *
	...
	*/
	...
	std::cout << "Inverse diagonal l2 norm: " << vec.l2_norm() << std::endl;
      ...
      if (Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0)
        {
          std::cout << "Coarse solver " << (int)timings[minlevel][1]
                    << " times: " << timings[minlevel][0] << std::endl;
                ...
                const GpuVector<Number2> &src) const
    {
      ...
      std::cout << "Norms: " << src.l2_norm() << " " << dst.l2_norm() << std::endl;
    }
