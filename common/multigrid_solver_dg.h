...
namespace multigrid
{
      ...
      , analytic_solution(boundary_values)
    {
...
#ifdef DEBUG
      std::vector<types::global_dof_index> dof_indices_act(fe_q.dofs_per_cell),
        dof_indices_mg(fe_q.dofs_per_cell);
      ...
      for (unsigned int l = 0; l < maxlevel + 1; ++l)
        {
          ...
          const auto numbering =
            DoFRenumbering::compute_matrix_free_data_locality(dof_handler_fe,
                                                              level_constraints,
                                                              mf_data);
          dof_handler_fe.renumber_dofs(l, numbering);
          if (l == maxlevel)
            dof_handler_fe.renumber_dofs(numbering);
        }
#ifdef DEBUG
      for (auto &cell : dof_handler_fe.active_cell_iterators())
        if (cell->is_locally_owned())
            ...
            if (level < maxlevel)
              {
                ...
                mg_mf_storage_level->reinit(
                  mapping, dof_handler_fe, level_constraints, QGauss<1>(fe_degree + 1), mf_data);
              }
            else
              {
                ...
                mg_mf_storage_level->reinit(mapping,
                                            std::vector<const DoFHandler<dim> *>(
                                              {&dof_handler_fe, &dof_handler}),
                                            std::vector<const AffineConstraints<double> *>(
                                              {&level_constraints, &dg_constraints}),
                                            std::vector<Quadrature<1>>({QGauss<1>(fe_degree + 1)}),
                                            mf_data);
                matrix_dg.reinit(mg_mf_storage_level, 1, &matrix[level]);
      ...
      // and one is for the homogeneous part in the v-cycle
      {
        std::vector<std::shared_ptr<const Utilities::MPI::Partitioner>> partitioners(
          dof_handler_fe.get_triangulation().n_global_levels());
        for (unsigned int level = minlevel; level <= maxlevel; ++level)
        ...
        }
      {
        typename PreconditionChebyshev<LaplaceOperatorCompactCombine<dim, fe_degree, Number>,
                                       ...
                                       JacobiTransformed<dim, fe_degree, Number>>::AdditionalData
          smoother_data;
        smoother_data.smoothing_range     = 20;
        smoother_data.degree              = degree_pre;
        smoother_data.eig_cg_n_iterations = 15;
        smoother_data.preconditioner.reset(
          new JacobiTransformed<dim, fe_degree, Number>(matrix_dg));
  ...
  private:
    ...
    void
    dg_v_cycle(const unsigned int my_n_cycles) const
    {
      for (unsigned int c = 0; c < my_n_cycles; ++c)
        {
          Timer time;
          ...
          time.restart();
          matrix_dg.prolongate_add_cg_to_dg(solution_update_dg, solution_update[maxlevel]);
    ...
    PreconditionChebyshev<LaplaceOperatorCompactCombine<dim, fe_degree, Number>,
                          VectorType,
                          JacobiTransformed<dim, fe_degree, Number>>
      smooth_dg;
