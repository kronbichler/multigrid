...
namespace multigrid
{
      ...
      for (unsigned int cell=0; cell<matrix_dp[level].get_matrix_free()->n_macro_cells(); ++cell)
        {
          ...
          for (unsigned int v=0; v<matrix_dp[level].get_matrix_free()->n_active_entries_per_cell_batch(cell); ++v)
            {
  ...
  public:
      ...
      for (unsigned int cell=0; cell<matrix[level].get_matrix_free()->n_macro_cells(); ++cell)
        {
          ...
          for (unsigned int v=0; v<matrix[level].get_matrix_free()->n_active_entries_per_cell_batch(cell); ++v)
            {
              global_error += local_error[v];
              global_volume += local_volume[v];
            }
    ...
    const LinearAlgebra::distributed::Vector<Number> &get_solution()
    {
      for (auto &i : inhomogeneous_bc[maxlevel])
        solution[maxlevel](i.first) = i.second;
      return solution[maxlevel];
          ...
          if (do_analyze)
            {
              ...
              if (Utilities::MPI::this_mpi_process(MPI_COMM_WORLD)==0)
                std::cout << "residual norm start level " << level << ": " << init_residual << std::endl;
              const double l2_error = compute_l2_error(level);
              if (Utilities::MPI::this_mpi_process(MPI_COMM_WORLD)==0)
                std::cout << "error start         level " << level << ": " << l2_error << std::endl;
              for (auto &i : inhomogeneous_bc[level])
    ...
    solve_cg()
    {
      ...
      solution_update = 0;
      solver_cg.solve(matrix[maxlevel], solution_update, rhs[maxlevel],
                      *this);
  ...
  private:
     ...
     */
    void v_cycle(const unsigned int level,
                 const bool outer_solution) const
    {
      if (level==minlevel)
        {
          Timer time;
