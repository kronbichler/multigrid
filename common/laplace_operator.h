...
namespace multigrid
{
  ...
  void
  print_time(const double time, const std::string &name, const MPI_Comm communicator)
  {
    ...
    const unsigned int                             level)
  {
    ...
    const IndexSet &locally_owned = this->data->get_dof_handler(0).locally_owned_mg_dofs(level);
  ...
  template <int dim, int fe_degree, typename number>
  void
  LaplaceOperator<dim, fe_degree, number>::extract_compressed_indices(
    const AffineConstraints<double> &constraints,
    const unsigned int               level)
  {
    if (fe_degree > 2)
            ...
            if (fe_degree > 2)
              {
                if (level == dealii::numbers::invalid_unsigned_int)
                  ...
                  typename dealii::DoFHandler<dim>::level_cell_iterator(
                    ...
                    ->get_active_or_mg_dof_indices(dof_indices);
                const unsigned int n_components =
                  this->data->get_dof_handler().get_fe().n_components();
  ...
  LaplaceOperator<dim, fe_degree, number>::evaluate_coefficient(
    const Function<dim> &coefficient_function)
  {
    ...
    merged_coefficient.resize(is_constant_coefficient ?
                                ...
                                this->data->n_cell_batches() * this->data->get_n_q_points());
    ...
    for (unsigned int cell = 0; cell < this->data->n_cell_batches(); ++cell)
      {
        ...
        if (is_constant_coefficient && this->data->get_mapping_info().get_cell_type(cell) < 2)
          {
          ...
          {
            fe_eval.reinit(cell);
            for (unsigned int q = 0; q < this->data->get_mapping_info()
                                           .cell_data[0]
                                           .descriptor[0]
  ...
  template <int dim, int fe_degree, typename number>
  void
  LaplaceOperator<dim, fe_degree, number>::do_quadrature_point_operation(
    FEEvaluation<dim, fe_degree, fe_degree + 1, 1, number> &phi_in,
    FEEvaluation<dim, fe_degree, fe_degree + 1, 1, number> &phi_out,
    const unsigned int                                      cell) const
  {
    ...
    const LinearAlgebra::distributed::Vector<number> &src) const
  {
    ...
    for (unsigned int i = 0; i < vmult_edge_constrained_indices.size(); ++i)
      {
        vmult_edge_constrained_values[i] = src.local_element(vmult_edge_constrained_indices[i]);
        const_cast<LinearAlgebra::distributed::Vector<number> &>(src).local_element(
          vmult_edge_constrained_indices[i]) = 0.;
      }

    // zero dst within the loop
    this->data->cell_loop(&LaplaceOperator::local_apply, this, dst, src, true);

    for (unsigned int i : this->data->get_constrained_dofs(0))
      dst.local_element(i) = src.local_element(i);

    for (unsigned int i = 0; i < vmult_edge_constrained_indices.size(); ++i)
      {
        dst.local_element(vmult_edge_constrained_indices[i]) = vmult_edge_constrained_values[i];
        const_cast<LinearAlgebra::distributed::Vector<number> &>(src).local_element(
          vmult_edge_constrained_indices[i]) = vmult_edge_constrained_values[i];
      }
  }
  ...
  template <int dim, int fe_degree, typename number>
  std::array<number, 4>
  LaplaceOperator<dim, fe_degree, number>::vmult_with_cg_update(
    const number                                      alpha,
    const number                                      beta,
    const LinearAlgebra::distributed::Vector<number> &r,
    LinearAlgebra::distributed::Vector<number> &      q,
  ...
  template <int dim, int fe_degree, typename number>
  void
  LaplaceOperator<dim, fe_degree, number>::adjust_ghost_range_if_necessary(
    const LinearAlgebra::distributed::Vector<number> &vec) const
  {
    if (vec.get_partitioner().get() == this->data->get_dof_info(0).vector_partitioner.get())
      return;
