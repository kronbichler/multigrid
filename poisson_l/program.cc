...
namespace multigrid
{
  ...
  {
  public:
    Solution(const bool use_hyper_l)
      : Function<dim>(1)
      , hyper_l(use_hyper_l)
    {}
  ...
  public:
    LaplaceProblem();
    void
    run(const unsigned int max_size, const unsigned int n_smoother, const bool use_hyper_l);

  private:
    void
    setup_system();
    std::pair<unsigned int, std::pair<double, double>>
    solve(const unsigned int n_smoother, const bool first_time);
    void
    output_results(const unsigned int cycle) const;
    void
    interpolate_boundary_values(const bool use_hyper_l);
  ...
  LaplaceProblem<dim, degree>::setup_system()
  {
    ...
    pcout << "Distribute DoFs & B.C.                " << time.wall_time() << "s" << std::endl;
    time.restart();
    ...
    std::vector<const DoFHandler<dim> *> dof_handlers(2, &dof_handler);
    {
      ...
      additional_data.tasks_parallel_scheme = MatrixFree<dim, full_number>::AdditionalData::none;
    ...
    setup_time += time.wall_time();
    pcout << "Setup matrix-free system              " << time.wall_time() << "s" << std::endl;
    time.restart();
    ...
    for (unsigned int level = 0; level < nlevels; ++level)
      {
        ...
        std::shared_ptr<MatrixFree<dim, vcycle_number>> mg_matrix_free_level(
          new MatrixFree<dim, vcycle_number>());
    ...
    setup_time += time.wall_time();
    pcout << "Setup matrix-free levels              " << time.wall_time() << "s" << std::endl;
    ...
    setup_time += time.wall_time();
    pcout << "MG build transfer time                " << time.wall_time() << "s" << std::endl;
  ...
  template <int dim, int degree>
  void
  LaplaceProblem<dim, degree>::interpolate_boundary_values(const bool use_hyper_l)
  {
    std::map<types::global_dof_index, double> boundary_values;
    VectorTools::interpolate_boundary_values(
      mapping, dof_handler, 0, Solution<dim>(use_hyper_l), boundary_values);
  ...
  template <int dim, int degree>
  std::pair<unsigned int, std::pair<double, double>>
  LaplaceProblem<dim, degree>::solve(const unsigned int n_smoother, const bool /*first_time*/)
  {
    Timer time;
    ...
    mg::Matrix<LinearAlgebra::distributed::Vector<vcycle_number>> mg_interface(
      mg_interface_matrices);

    Multigrid<LinearAlgebra::distributed::Vector<vcycle_number>> mg(
    ...
    pcout << "MG build smoother time                " << time.wall_time() << "s" << std::endl;
    ...
    std::pair<unsigned int, std::pair<double, double>> stats(
      solver_control.last_step(), std::pair<double, double>(time.wall_time(), 0));
    time.reset();
    time.start();
    solution_update = 0;
    cg.solve(system_matrix, solution_update, system_rhs, preconditioner);
    stats.second.second = time.wall_time();
    pcout << "Time solve (" << stats.first << " iterations)             " << stats.second.second
          << "s  convergence rate "
  ...
  template <int dim, int degree>
  void
  LaplaceProblem<dim, degree>::output_results(const unsigned int cycle) const
  {
  ...
  void
  create_tria(Triangulation<2> &tria, const bool)
  {
  ...
  void
  create_tria(Triangulation<3> &tria, const bool use_hyper_l)
  {
    if (use_hyper_l)
      {
        GridGenerator::hyper_L(tria);
        for (Triangulation<3>::cell_iterator cell = tria.begin(); cell != tria.end(); ++cell)
          for (unsigned int f = 0; f < GeometryInfo<3>::faces_per_cell; ++f)
            if (cell->face(f)->at_boundary() && std::abs(cell->face(f)->center()[0] + 1) < 1e-10)
  ...
  template <int dim, int degree>
  void
  LaplaceProblem<dim, degree>::run(const unsigned int max_size,
    ...
    for (unsigned int cycle = 0; cycle < 20; ++cycle)
      {
        ...
        if (cycle == 0)
        ...
        else
          {
            ...
            parallel::distributed::GridRefinement::refine_and_coarsen_fixed_number(
              triangulation, estimated_error_per_cell, (use_hyper_l ? 0.15 : 0.3), 0.03);
            triangulation.prepare_coarsening_and_refinement();
            parallel::distributed::SolutionTransfer<dim,
                                                    LinearAlgebra::distributed::Vector<full_number>>
        ...
        system_matrix.compute_residual(system_rhs,
                                       solution,
  ...
  public:
                   ...
                   const bool         use_hyper_l)
    {
      ...
      LaplaceRunTime<dim, (min_degree <= max_degree ? (min_degree + 1) : min_degree), max_degree> m(
        target_degree, max_size, n_smoother, use_hyper_l);
    }
  };
} // namespace multigrid
      ...
      if (argc == 1)
        {
          if (Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0)
            std::cout << "Expected at least one argument." << std::endl
                      << "Usage:" << std::endl
                      << "./program degree maxsize n_smoother hyper_l" << std::endl
